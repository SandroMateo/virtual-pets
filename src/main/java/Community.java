...
public class Community implements DatabaseManagement{
  ...
  public void save(){
    try(Connection con = DB.sql2o.open()){
      ...
      this.id = (int) con.createQuery(sql,true).addParameter("name",this.name).addParameter("description",this.description).executeUpdate().getKey();
    }
  }
  ...
  public void addPerson(Person person){
    try(Connection con = DB.sql2o.open()){
      ...
      con.createQuery(sql).addParameter("community_id",this.getId()).addParameter("person_id",person.getId()).executeUpdate();
    }
  }
